...
def _load_products_from_url():
    ...
    try:
        ...
        log.warning(f"[RAG] URL load failed: {e}")
        return _load_json_local(LOCAL_PRODUCTS)
...
def _join(xs)->str:
    if not xs: return ""
    if isinstance(xs, (list, tuple)): return " ".join(_norm(str(x)) for x in xs)
    return _norm(str(xs))
...
class MiniRAG:
    def __init__(self):
        ...
        self.index_docs=[]; self.meta=[]
        self.products=[]; self.combos=[]; self.symptoms=[]
        self._load_all()

    # ---- Public API ----
  def reload(self)->Dict[str,Any]:
        self._load_all()
        return {"ok":True, "counts":{"products":len(self.products),"combos":len(self.combos),"symptoms":len(self.symptoms)}}

    def search(self, query:str, topk:int=5)->List[Dict]:
        if not query.strip() or not self.index_docs: return []
    ...
    # Convenience getters
    def get_product(self, sku):  return next((p for p in self.products if p.get("sku")==sku), None)
    def get_combo(self, cid):    return next((c for c in self.combos if c.get("id")==cid), None)
    def get_symptom(self, sid):  return next((s for s in self.symptoms if s.get("id")==sid), None)
    ...
    def _load_all(self):
        ...
        log.info(f"[RAG] P/C/S={len(self.products)}/{len(self.combos)}/{len(self.symptoms)} docs={len(self.index_docs)}")
        ...
        # Combos → one doc per combo
         for c in self.combos:
                docs.append(" | ".join([
                    _norm(c.get("name","")), _join(c.get("targets",[])),
                    _norm(c.get("protocol","")), _join([i.get("sku") for i in c.get("items",[])]),
                    _norm(c.get("notes",""))
                ]))
        ...
        for s in self.symptoms:
            docs.append(" | ".join([
                _norm(s.get("symptom","")), _join(s.get("keywords",[])),
                _join(s.get("triage_questions",[])), _join(s.get("red_flags",[])),
